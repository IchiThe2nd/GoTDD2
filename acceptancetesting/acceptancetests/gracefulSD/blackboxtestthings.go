...
func LaunchTestProgram(port string) (cleanup func(), sendInterrupt func() error, err error) {
	...
	if err != nil {
		cleanup() // not listening but might still be runnuinpg
		return nil, nil, err
	}
...
func runServer(binName string, port string) (sendInterrupt func() error, kill func(), err error) {
	...
	if err := cmd.Start(); err != nil {
		return nil, nil, fmt.Errorf("cannot trun temp convberter %s ", err)
	}
	kill = func() {
		_ = cmd.Process.Kill()
	}
...
func waitForServerListening(port string) error {
	...
	return fmt.Errorf("nothing seems to be listing to localhost:%s", port)

}
func randomString(n int) string {
	var letters = []rune("abcdefghijhklmnopqrstuvwxyz")
	s := make([]rune, n)
	for i := range s {
